...
# GBM prints out a LOT of unnecessary information for this report..
suppressWarnings(gbmModel <- trainer("gbm"))
rfModel <- trainer("rf")
...
predCART <- predict(cartModel, newdata = trainingSet)
cmCART <- confusionMatrix(predCART, testingSet$classe)
predGBM <- predict(gbmModel, newdata = trainingSet)
cmGBM <- confusionMatrix(predGBM, testingSet$classe)
predRF <- predict(rfModel, newdata = trainingSet)
cmRF <- confusionMatrix(predRF, testingSet$classe)
```

```{r kable_table}
library(knitr)
Model <- c("CART","GBM", "RF")
Accuracy <- c(cmCART$overall[1], cmGBM$overall[1], cmRF$overall[1])
theTable <- data.frame(Model, Accuracy)
